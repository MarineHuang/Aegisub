...
void AudioOp::extract_audio(const agi::fs::path &src_video, 
     ...
     AUDIO_OP_ERRORS &err) 
{
     ...
     try{
          ...
          AudioOpWrapper op_wrapper(dst_audio);
          
          int err_code = 0;
          char errors[1024];
  
          // 打开输入的视频文件，分配文件操作上下文
          if((err_code = avformat_open_input(&op_wrapper.fmt_ctx, src_video.string().c_str(), nullptr, nullptr)) < 0) {
               av_strerror(err_code, errors, 1024);
                 av_log(nullptr, 
                    AV_LOG_DEBUG, 
                    "Could not open source file: %s, %d(%s)\n", 
                    src_video.string().c_str(),
                    err_code,
                    errors
                    );
          ...
          if((err_code = avformat_find_stream_info(op_wrapper.fmt_ctx, nullptr)) < 0) {
               ...
               av_log(NULL, 
                    AV_LOG_DEBUG, 
                    "failed to find stream information: %s, %d(%s)\n",
                    src_video.string().c_str(),
                    err_code,
                    errors
                    );
          ...
          // 显示视频流信息
          av_dump_format(op_wrapper.fmt_ctx, 0, src_video.string().c_str(), 0);
          ...
          while((av_read_frame(op_wrapper.fmt_ctx, &op_wrapper.pkt)) >= 0) {
               ...
               if(op_wrapper.pkt.stream_index == audio_stream_index) {
                    ...
                    op_wrapper.write(adts_header_buf, 7);
                    op_wrapper.write((char*)op_wrapper.pkt.data, op_wrapper.pkt.size); 
               }
          }
     }
     catch (const agi::fs::WriteDenied& e)
     {
         av_log(nullptr, 
               AV_LOG_DEBUG, 
