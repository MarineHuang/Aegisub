...
HttpClient::HttpResponseOutcome CurlHttpClient::makeRequest(const HttpRequest &request) {
  ...
  if (request.method() != HttpRequest::Get) {
    ...
    if (request.hasBody()) {
      curl_easy_setopt(curlHandle_, CURLOPT_POSTFIELDS, request.getBody().c_str());
      curl_easy_setopt(curlHandle_, CURLOPT_POSTFIELDSIZE, request.getBodySize());
  ...
  if (list != NULL) {
    ...
    curl_easy_cleanup(curlHandle_);
		return std::move(HttpResponseOutcome(std::move(response)));
	}

  curl_easy_cleanup(curlHandle_);
  
	return std::move(HttpResponseOutcome(Error("NetworkError", "Failed to connect to host or proxy.")));
}
